...
export default function TeamsByDivision() {
  ...
  const TeamCard = ({ team }: { team: Team }) => (
    <div
      onMouseEnter={() => { setHoveredTeam(team.id); fetchPlayers(team.id); }}
    ...
    >
      <Card className="flex flex-col items-center justify-between p-4 h-64 relative overflow-hidden hover:shadow-lg transition">
        ...
        {/* Group */}
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">Group: {team.group}</p>
        ...
        >
          ...
          {playersByTeam[team.id]?.length ? (
            ...
            <p className="text-sm text-gray-300 pointer-events-auto">No players found</p>
          )}
        </motion.div>
      </Card>
    </div>
        ...
        {divisions.map(div => (
          <button
            ...
            className={`px-6 py-3 rounded-lg text-lg font-semibold transition
              ${selectedDivId === div.id ? "bg-blue-600 text-white shadow-lg" : "bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200 hover:bg-blue-500 hover:text-white"}`}
          >
            {div.name}
          </button>
        ))}
            ...
            {teams.filter(t => t.div === selectedDivision.name).length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {teams
                  ...
                  .map(team => (
                    isMobile ? <TeamCard key={team.id} team={team} /> :
                      <Link key={team.id} href={`/team/${team.id}`}><TeamCard team={team} /></Link>
                  ))}
              </div>
            )}
          </motion.div>
