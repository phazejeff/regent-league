...
def get_matches_cc(div: str | None = None, session: Session = Depends(get_session)) -> List[CCMatch]:
    ...
    if div:
        statement = statement.where(Upcoming.division == div)
    results = session.exec(statement).all()
    response = [CCMatch.convert_upcoming_to_cc(result, REG_SEASON_END) for result in results]
    ...
    if div:
        ...
        statement = statement.where(Team.div == div)
    results = session.exec(statement).all()
    response = response + [CCMatch.convert_finished_match_to_cc(result, REG_SEASON_END) for result in results]
